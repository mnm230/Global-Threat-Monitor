...
- **Cyber Threats Panel:** ME-only cyber threat intelligence. Sources: 7 RSS feeds (BleepingComputer, Hacker News, The Record, Dark Reading, Unit42, Check Point, SecureList) + OTX AlienVault pulses. Articles pre-filtered by ME keyword relevance, then GPT-4o-mini extracts strictly ME-only events. Post-extraction hard-filter enforces ME country whitelist (20+ countries with alias normalization for ISO/variant names). Known ME APT groups tracked: APT33/Elfin, APT34/OilRig, APT35/Charming Kitten, MuddyWater, Moses Staff, Agrius, etc. Fallback: 8 intelligence-grade supplemental events from known active ME campaigns when real data is sparse (<3 events). 10s cache TTL.
...
**Cache TTLs:** All caches ≤10s (NEWS 10s, X 120s, OSINT_RSS 60s, GDELT 10s, FX 10s, COMMODITY 10s, EQ 10s, THERMAL 10s, CYBER 10s, TELEGRAM 10s, AI_CLASSIFY 10s, AI_BRIEF 10s, OREF 0s). **Global cache flush every 15 minutes** clears all caches to ensure fresh data. FETCH_COOLDOWN_MS=0 (ADS-B throttled by SSE interval). Client staleTime=0, refetchOnWindowFocus=true. Alert history limit: 2000 entries.
