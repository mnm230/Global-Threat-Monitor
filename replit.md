...
- **Panel System:** Features 15 panels (`intel`, `map`, `telegram`, `events`, `radar`, `alerts`, `markets`, `ew`, `infra`, `livefeed`, `alertmap`, `analytics`, `osint`, `sitrep`, `attackpred`) in a customizable, resizable grid (react-grid-layout, 12-col, 110px row height). Mobile uses single-panel view, tablet uses 2-column CSS grid.
