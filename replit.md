...
**UI/UX Decisions:**
- **Theme:** Glassmorphic military command-center aesthetic with lightened dark theme. Background `hsl(220 30% 12%)`, card `hsl(220 28% 15%)`, borders `hsl(185 25% 22%)`. Cyan primary accent (`hsl(185 100% 42%)`), semi-transparent panel backgrounds with `backdrop-filter: blur`. 1px thin grid lines between panels. Gradient accent lines on panel headers. Subtle glow effects on interactive elements. Header/sidebar/ticker/status bar/bottom sheet all glass. React-grid-layout 1px margins. Inline panel headers use `hsl(220 28% 13% / 0.9)`.
...
- **Panel System:** Features 16 panels (`intel`, `map`, `telegram`, `events`, `radar`, `alerts`, `markets`, `ew`, `infra`, `livefeed`, `alertmap`, `analytics`, `osint`, `sitrep`, `attackpred`, `rocketstats`) in a customizable, resizable grid (react-grid-layout, 12-col, 110px row height). Mobile uses single-panel view, tablet uses 2-column CSS grid.
