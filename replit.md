...
- **Panel System:** Features customizable, resizable panels within a React-grid-layout. Each panel is extracted into its own file under `client/src/components/panels/` (13 panel files), with `dashboard.tsx` serving as the orchestrator (~1,400 lines) handling grid layout and panel routing. All shared hooks are extracted to `client/src/hooks/` (use-sse, use-alert-sound, use-notifications, use-anomaly-detection, use-escalation, use-correlations). Shared panel UI (PanelHeader, FreshnessBadge, PanelMinimizeButton, PanelMaximizeButton, FeedFreshnessContext) lives in `panel-chrome.tsx`. Shared utilities (timeAgo, headingToCompass, formatPrice, getThreatLevel, sendNotification, generateExportReport) live in `client/src/lib/dashboard-utils.ts`. Shared types/constants/config in `client/src/lib/dashboard-types.ts`. Audio alerts in `client/src/lib/audio-alerts.ts`. Supporting UI components (EscalationBanner, EventTimeline, LiveClock, NewsTicker, SirenBanner, FloatingWindow, PanelSidebar, PanelErrorBoundary, ResizeHandle, AlertMapPanel, OsintTimelinePanel, LiveFlightTracker, MapSection, AlertHistoryTimeline) extracted to `client/src/components/`. Overlay components (NotesOverlay, WatchlistOverlay, AlertHistoryOverlay, SettingsOverlay, LayoutPresetsDropdown) extracted to `client/src/components/overlays/`.
...
- **AI Intelligence:** AI world brief panel with risk assessment and forecasting, and an AI Attack Predictor panel using Anthropic Claude for probabilistic attack predictions. AI Prediction panel uses only real alert data for statistical pattern analysis — no synthetic baselines, no speculative actor attributions, no inflated confidence floors. Shows "Insufficient Data" warning when <3 alerts exist in the last 6 hours. Includes disclaimer: "Statistical pattern analysis — not confirmed intelligence."
...
- **UX Polish:** Panel mount fade animation (`panel-enter` CSS, opacity-only to avoid RGL transform conflicts). `ScrollShadow` component on all scrollable panels provides top/bottom gradient indicators (with MutationObserver for dynamic content). Skeleton shimmer loading state shown while panels initialize (`skeleton-line` CSS animation).
