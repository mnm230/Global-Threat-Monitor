...
11. **Israel Red Alert (Tzeva Adom)** - Multi-country alert system (57 cities across 12 countries: Israel, Lebanon, Iran, Syria, Iraq, Saudi Arabia, Yemen, UAE, Jordan, Kuwait, Bahrain, Qatar). Real Pikud HaOref API integration (`oref.org.il/WarningMessages/alert/alerts.json`) with 5s cache, 4s timeout, graceful fallback to simulated data. `source` field on RedAlert schema ('live'|'sim'). Priority sorting: active first, live API alerts prioritized, lowest countdown first, threat severity (missiles > rockets > UAV > aircraft). Visual urgency tiers: critical (red pulse bg, countdown <= 15s), urgent (amber, <= 45s), warning, standard, expired. Source badges (green "API" / gray "SIM") on each alert. Red glow border on panel when alerts active. Country/region grouped with sticky headers, countdown timers, filters by threat type and country. Trilingual support (EN/AR/HE).
12. **Alert Sound System** - Web Audio API sine-wave tone on new red alerts/sirens, with sound toggle
13. **Panel Close/Reopen** - 11 closeable panels with reopen tabs in status bar
15. **YouTube Live Feed Panel** - Embedded YouTube live stream (default: bNyUyrR0PHo). LIVE badge, configurable URL via settings gear button (parses YouTube URLs to extract video ID). Autoplay muted, picture-in-picture support. Panel ID: `livefeed`.
...
31. **Responsive Layout** - Three breakpoints: Mobile (<768px) single-column scrollable stack, Tablet (768-1199px) 2-column scrollable grid with map spanning full width, Desktop (>=1200px) fixed two-row resizable panel layout. Hamburger menu on mobile/tablet, full toolbar on desktop. Page-level scrolling on smaller screens.
